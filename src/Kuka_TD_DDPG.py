...
class KukaTD3Agent:
    def __init__(self, state_size, action_size,
        ...
        self.actor = TD3Actor(self.state_size, self.action_size, self.replacement,
                              self.actor_lr, self.upper_bound, self.feature)

        # Two Critic networks in TD3
        self.critic_one = TD3Critic(self.state_size, self.action_size, self.replacement,
                                    self.critic_lr, self.gamma, self.feature)
        self.critic_two = TD3Critic(self.state_size, self.action_size, self.replacement,
                                    self.critic_lr, self.gamma, self.feature)
    ...
    def experience_replay(self):
        ...
        if self.train_step % self.policy_decay == 0:
            actor_loss = self.actor.train(s_batch, self.critic_one)
            self.actor_loss = actor_loss
        self.train_step += 1

        return actor_loss, np.minimum(critic_one_loss, critic_two_loss)
    ...
    def save_model(self, path, actor_filename, critic_filename,
        ...
        critic_file_one = path + 'one_' + critic_filename
        critic_file_two = path + 'two_' + critic_filename
        ...
        self.critic_one.model.save_weights(critic_file_one)
        self.critic_two.model.save_weights(critic_file_two)
