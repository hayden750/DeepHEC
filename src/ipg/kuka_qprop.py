...
class Actor:
    ...
    def train(self, states, actions, ls, n, critic):
        ...
        with tf.GradientTape() as tape:
            ...
            log_prob = new_pi.log_prob(tf.squeeze(actions))
            p1 = log_prob * ls

            a = self.model(states)
            c = critic.model([states, a])
            p2 = n * a * c

            actor_loss = -tf.math.reduce_mean(K.sum(p1 + p2))
...
class QPROPAgent:
    ...
    def replay(self, states, actions, rewards, dones, next_states):
        ...
        # Based on algorithm at https://arxiv.org/pdf/1611.02247.pdf

        # 1. Take gradient steps on Critic (Qw) using buffer and current policy
        ...
        # 2. Fit baseline using collected experience
        # Update baseline
        _, returns, v_values = self.compute_advantages(rewards, states, next_states, dones)
        v_loss = self.baseline.train(states, returns)
        v_loss_list.append(v_loss)

        # 3. Compute Advantages Estimate GAE
        advantages, _, _ = self.compute_advantages(rewards, states, next_states, dones)

        # 4. Compute Critic-based advantages
        cb_advantages = self.compute_adv_bar(states, actions)

        # 5. Set control variate value n
        advantages = tf.stack([advantages, advantages, advantages], axis=1)
        # p1 = np.cov(advantages, cb_advantages)
        # p2 = np.var(cb_advantages)
        # n = p1 / p2
        n = 1

        # 6. Compute and center learning signals ls = adv - (n * adv_bar)
        ls = advantages - (n * cb_advantages)

        # 7. Update Actor
        a_loss = self.actor.train(states, actions, ls, n, self.critic)
